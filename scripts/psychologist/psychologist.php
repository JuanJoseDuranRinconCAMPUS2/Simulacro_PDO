...
class psychologist extends connect{
    ...
    private $message;
    use getInstance;
    function __construct(private $id = 0, private $id_route = 1, private $id_staff = 1, private $id_academic_area_psycologist = 1, private $id_position = 1, private $id_team_educator = 1){parent::__construct();}
    public function postPsychologist(){
        try {
            $res = $this->conx->prepare($this->queryPost);
            $res->bindValue("ID_fStaff", $this->id_staff);
            $res->bindValue("ID_fRoute", $this->id_route);
            $res->bindValue("ID_fAcademicArea", $this->id_academic_area_psycologist);
